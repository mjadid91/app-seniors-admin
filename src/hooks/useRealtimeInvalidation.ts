 ...
 * IMPORTANT: Ce hook accepte maintenant l'état d'authentification pour différer la connexion (Lazy Subscription).
 */
export const useRealtimeInvalidation = (isAuthenticated: boolean = false) => {
  const queryClient = useQueryClient();
  const channelRef = useRef<any>(null);
  const isSubscribedRef = useRef(false);

  useEffect(() => {
    // 🛑 VERROU D'ARCHITECTURE : On bloque tout si l'utilisateur n'est pas authentifié
    if (!isAuthenticated) {
      console.log('⏳ [Realtime] En attente de l\'authentification pour activer les WebSockets...');
      return;
    }
    ...
    console.log('🔌 [Realtime] Authentification validée, lancement de la connexion globale...');
    ...
    channelRef.current = supabase
        ...
        .subscribe((status) => {
          ...
          if (status === 'SUBSCRIBED') {
            ...
            console.log('✅ [Realtime] Écoute temps réel activée pour toutes les tables');
          }
        });
    ...
    return () => {
      if (channelRef.current) {
        ...
        console.log('🛑 [Realtime] Écoute temps réel désactivée');
      }
    };
  }, [queryClient, isAuthenticated]); // <-- Il est très important que isAuthenticated soit ici
