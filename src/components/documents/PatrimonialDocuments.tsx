...
// ✅ CORRECTION 1 : On importe l'interface PatrimonialDocument directement depuis ton hook
import { usePatrimonialDocuments, PatrimonialDocument } from "./usePatrimonialDocuments";
...
const PatrimonialDocuments = () => {
  ...
  // ✅ CORRECTION 2 : On utilise l'interface importée
  const [selectedDocument, setSelectedDocument] = useState<PatrimonialDocument | null>(null);
  ...
  // ✅ CORRECTION 3 : Utilisation de PatrimonialDocument
  const canDownload = (doc: PatrimonialDocument) => {
  ...
  const canViewContent = (doc: PatrimonialDocument) => {
    // Même règle que pour le téléchargement
    return canDownload(doc);
  };

  const handleViewClick = (doc: PatrimonialDocument) => {
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tr className="border-b border-slate-200">
                ...
                <th className="text-left py-3 px-4 font-medium text-slate-600">Senior</th>
              ...
              {documents.length === 0 ? (
                  ...
                  documents.map((doc) => (
                      ...
                      <span className="text-slate-700">
                        {doc.Seniors?.Utilisateurs ?
                            `${doc.Seniors.Utilisateurs.Prenom} ${doc.Seniors.Utilisateurs.Nom}` :
                            'Non assigné'
                        }
                      </span>
                        </td>
