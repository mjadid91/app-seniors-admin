...
const NewLoginPage = () => {
  ...
  useEffect(() => {
    if (!loading && user) {
      console.log('NewLoginPage: User already authenticated, redirecting to dashboard');
      navigate("/dashboard");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('NewLoginPage: Attempting login for:', email);
      const result = await signIn(email.trim(), password);

      if (!result.success) {
        setError(result.error || "Erreur de connexion");
      } else {
        console.log('NewLoginPage: Login successful');
        navigate("/dashboard");
      }
    } catch (err) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="mt-6 p-3 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="text-xs text-blue-700 space-y-1">
                <div>Utilisez l'email et mot de passe de votre compte Supabase</div>
                <div>Confirmez votre email si vous venez de vous inscrire</div>
              </div>
            </div>
          </CardContent>
        </Card>
